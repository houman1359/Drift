% Sweep noise correlation rho for both synaptic and excitability modes
clear all
close all
...
% Set to 0 for independent excitability noise (matches analytical assumption).
% Set to 1 for correlated excitability noise using rho_noise.
excit_corr_excitability = 0;
% Optional: excitability correlation coefficient (independent of rho_noise)
% If empty, it will match rho_noise inside the core function.
excit_rho_excitability = [0];
...
for mi = 1:length(modes)
    ...
    r_noise_mean = nan(length(rhos),1);
    r_noise_sem = nan(length(rhos),1);
    ...
    for ri = 1:length(rhos)
        ...
        for rep = 1:Nreps
            ...
            rn = corr(f.pair_noise_corrs, f.pair_drift_vars, 'Rows','pairwise');
            rs = corr(f.pair_signal_corrs, f.pair_drift_vars, 'Rows','pairwise');
            if ~exist('r_noise_reps','var') || numel(r_noise_reps) < Nreps
                r_noise_reps = nan(Nreps,1);
        ...
        r_noise_mean(ri) = nanmean(r_noise_reps);
        r_noise_sem(ri) = nanstd(r_noise_reps)/sqrt(Nreps);
    ...
    results.(mode).r_noise_mean = r_noise_mean;
    results.(mode).r_noise_sem = r_noise_sem;
    ...
    figure;
    subplot(1,2,1); errorbar(rhos, r_noise_mean, r_noise_sem, '-o','LineWidth',2); xlabel('\rho'); ylabel('corr(pair noise corr, pair drift var)'); title(['Pair drift vs noise corr (',mode,')'],'Interpreter','none');
