...
rho_noise_values = [0.0, 0.2, 0.4, 0.6, 0.8]  # Different noise correlations
N_runs = 100  # Number of simulations per rho_zeta and rho_noise
...
for rho_zeta in rho_zeta_values:
    for rho_noise in rho_noise_values:
        print(f"Running simulations for rho_zeta = {rho_zeta}, rho_noise = {rho_noise}")
        
        # Define covariance matrices based on rho_noise
        cov_noise = np.array([[sigma_w**2, rho_noise * sigma_w**2],
                              [rho_noise * sigma_w**2, sigma_w**2]])
        cov_M_noise = np.array([[sigma_m**2, rho_noise * sigma_m**2],
                                [rho_noise * sigma_m**2, sigma_m**2]])
        
        # Temporary lists to accumulate results for current rho_zeta and rho_noise
        temp_mean_drift_syn = []
        temp_mean_drift_exc = []
        ...
        for run in range(N_runs):
            ...
            for t in range(T):
                ...
                # Synaptic Noise Model: Correlated noise
                xi_w_syn = multivariate_normal.rvs(mean=[0, 0], cov=cov_noise)
                xi_M_syn = multivariate_normal.rvs(mean=[0, 0], cov=cov_M_noise)
        ...
        # After all runs for current rho_zeta and rho_noise, append to the main results
        results['rho_zeta'].extend([rho_zeta]*rho_noise_values.__len__()*N_runs)
        results['rho_noise'].extend([rho_noise]*N_runs for rho_noise in rho_noise_values)
        # To correctly extend with both rho_zeta and rho_noise, adjust accordingly
        for rho_noise in rho_noise_values:
            results['rho_zeta'].extend([rho_zeta]*N_runs)
            results['rho_noise'].extend([rho_noise]*N_runs)
...
for idx, (x_label, y_label, title) in enumerate(scatter_pairs):
    ...
    x_data = df_results[x_label]
    y_data = df_results[y_label]
    
    # Compute Pearson correlation coefficient
    corr_coef, p_value = pearsonr(x_data, y_data)
    
    # Create scatter plot
    ax.scatter(x_data, y_data, alpha=0.3, edgecolor='k', linewidth=0.2)
